...
(function (ask) {
    ...
    (function (apps) {
        ...
        (function (background) {
            ...
            function resetFocusSetOnRestartForEdge() {
                if (!BrowserUtils.isEdgeChromium())
                    return;
                var stateKeyForSetFocusOnRestart = "setFocusOnRestart";
                chrome.storage.local.get(stateKeyForSetFocusOnRestart, function (edgeState) {
                    if (Object.keys(edgeState).length) {
                        chrome.tabs.query({}, function (tabs) {
                            if (tabs.length === 1)
                                return;
                            edgeState[stateKeyForSetFocusOnRestart] = false;
